...
iterations = 2000;
velocity = 0.5:0.005:v_max;
e = zeros(iterations, size(velocity,2));
index = 1;
for v = velocity
    v % print the current velocity
...
%%
plot(velocity, mean(e),'b')
title("Localisation error vs Vehicle A velocity (Vehicle distance 500m)");
xlabel("Vehicle A velocity (m/s)");
ylabel("Average error (m)");
%%
hold on;
onepercent = sort(e,'descend');
onepercent = onepercent(1:0.01*iterations,:);
plot(velocity, mean(onepercent),'b:')
